...
function NumberOfDiscIntersections(A) {
    ...
    for (var i = 0; i < A.length; i++) {
        ...
        var counter = 0;
        while (counter < A[i] * 2 && 0 <= i - counter || i + counter <= A.length) {
            counter = counter + 1;
            ...
            if (A[i + counter] < A[i] && counter <= A[i] + A[i + counter]) {
...
console.log("NumberOfDiscIntersections([1,2147483647,0]); expect 2: " + NumberOfDiscIntersections([1, 2147483647, 0]));
// Codility Testing
